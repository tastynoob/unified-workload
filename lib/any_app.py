...
def _validate_elf(path: Path, arch: str) -> None:
    if not path.is_file():
        raise BuildError(f"--any-elf is not a regular file: {path}")
    with path.open("rb") as elf_file:
        header = elf_file.read(20)
    if len(header) < 20 or header[:4] != b"\x7fELF":
        raise BuildError(f"--any-elf is not an ELF file: {path}")
    if header[4] != 2:
        raise BuildError(f"--any-elf must be ELF64: {path}")
    if header[5] not in (1, 2):
        raise BuildError(f"--any-elf has an unsupported byte order: {path}")
    byte_order = "<" if header[5] == 1 else ">"
    elf_type = struct.unpack(f"{byte_order}H", header[16:18])[0]
    if elf_type not in (2, 3):
        raise BuildError(f"--any-elf is not an executable ELF: {path}")
    ...
    if expected is not None and machine != expected:
        raise BuildError(
            f"--any-elf machine {machine} does not match architecture {arch}"
        )


def _parse_mapping(value: str) -> FileMapping:
    if "=" not in value:
        raise BuildError("--any-file must use HOST=GUEST syntax")
    host, guest = value.split("=", 1)
    if not host or not guest:
        raise BuildError("--any-file must use non-empty HOST=GUEST paths")
    return FileMapping(
        _host_path(host, "--any-file"),
        _guest_path(guest, "--any-file", allow_root=True),
    )


def load_config(ctx: BuildContext) -> AnyConfig:
    if ctx.args.any_elf is None:
        raise BuildError("the any workload requires --any-elf")
    elf = _host_path(str(ctx.args.any_elf), "--any-elf")
    _validate_elf(elf, ctx.arch)

    return AnyConfig(
        elf=elf,
        mappings=[_parse_mapping(value) for value in ctx.args.any_file],
    )
...
class ManifestBuilder:
    ...
    def _check_path(self, path: PurePosixPath, kind: str) -> None:
        if not path.is_absolute() or ".." in path.parts:
            raise BuildError(f"invalid guest path in --any-file mapping: {path}")
        if any(character.isspace() for character in str(path)):
            raise BuildError(f"guest path must not contain whitespace: {path}")
        if path in RESERVED_PATHS:
            raise BuildError(f"--any-file conflicts with reserved path: {path}")
        existing = self.entries.get(path)
        if existing is not None and not (kind == "dir" and existing.startswith("dir ")):
            raise BuildError(f"duplicate --any-file guest path: {path}")
    ...
    def add_parents(self, path: PurePosixPath) -> None:
        for parent in reversed(path.parents):
            ...
            if parent in RESERVED_PATHS:
                raise BuildError(f"--any-file conflicts with reserved path: {parent}")
            existing = self.entries.get(parent)
            if existing is not None and not existing.startswith("dir "):
                raise BuildError(f"--any-file parent is not a directory: {parent}")
    ...
    def add_mapping(self, mapping: FileMapping) -> None:
        ...
        if not source.is_dir():
            raise BuildError(f"--any-file supports only files and directories: {source}")
        ...
        for root, directories, files in os.walk(source, followlinks=False):
            ...
            for name in files:
                ...
                if path.is_symlink():
                    ...
                    raise BuildError(f"unsupported --any-file entry: {path}")
